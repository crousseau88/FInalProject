...
export class BootcampService {
  ...
  getBootcampsByKeyword(keyword: string) {
    return this.http
      .get<Bootcamp[]>(this.url + 'bootcamps/search/' + keyword)
      .pipe(
        catchError((err: any) => {
          console.log(err);
          return throwError(
    ...
    return this.http
      ...
      .pipe(
        catchError((err: any) => {
          console.log(err);
          return throwError(
            () => new Error('bootcampService: error POSTING REVIEW' + err)
          );
        })
      );
  }

  createAdvice(reviewId: number, advice: BootcampAdvice) {
    return this.http
      .post<BootcampAdvice>(
        this.url + 'advice/' + reviewId,
        advice,
        this.getHttpOptions()
      )
      .pipe(
        catchError((err: any) => {
          console.log(err);
          return throwError(
            () => new Error('bootcampService: error POSTING ADVICE' + err)
          );
        })
      );
  }
