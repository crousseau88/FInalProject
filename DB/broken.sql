...
CREATE TABLE IF NOT EXISTS `bootcamp_review` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `user_id` INT NOT NULL DEFAULT 3,
  `bootcamp_id` INT NOT NULL DEFAULT 1,
...
CREATE TABLE IF NOT EXISTS `bootcamp_advice` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `bootcamp_review_id` INT NOT NULL DEFAULT 1,
...
CREATE TABLE IF NOT EXISTS `tool` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `bootcamp_advice_id` INT NOT NULL DEFAULT 1,
...
CREATE TABLE IF NOT EXISTS `direct_message` (
  `id` INT NOT NULL AUTO_INCREMENT,
  `sender_id` INT NOT NULL DEFAULT 3,
  `recipient_id` INT NOT NULL DEFAULT 2,
  `date` DATETIME NULL,
  `content` TEXT NULL,
  `enabled` TINYINT NULL,
  PRIMARY KEY (`id`),
...
START TRANSACTION;
USE `mentormedb`;
INSERT INTO `bootcamp` (`id`, `name`, `website`, `image_url`, `length_weeks`, `description`, `bootcampcol`) VALUES (1, 'Skill Distillery', 'https://skilldistillery.com/', 'https://skilldistillery.com/wp-content/uploads/2016/02/skilldistillery_website.png', 16, 'The key is Java. Java is one of the most widely used programming languages and has been for over 15 years. Our parent company, Batky-Howell, has trained over 40,000 developers and has been teaching Java since it was introduced.  Leveraging this background we were able to create a unique curriculum that not only gives you greater job opportunities but also reduces your competition.', NULL);
INSERT INTO `bootcamp` (`id`, `name`, `website`, `image_url`, `length_weeks`, `description`, `bootcampcol`) VALUES (2, 'Hack Reactor', 'https://www.hackreactor.com/', 'https://www.mooclab.club/attachments/hack-reactor-logo-ii-jpg.576/', 12, 'Our coding bootcamps are challenging, life-changing, and designed to fit your schedule and skill level. We train students using a computer science and coding curriculum that models the exciting work being done in the software engineering industry. We\'ll help you launch the career you crave.\n\nLed by passionate and experienced instructors, engineers, and career advisors, our graduates become software engineers that companies around the world want to hire.', NULL);
INSERT INTO `bootcamp` (`id`, `name`, `website`, `image_url`, `length_weeks`, `description`, `bootcampcol`) VALUES (3, 'Code Platoon', 'https://www.codeplatoon.org/', 'https://www.codeplatoon.org/wp-content/uploads/2018/10/code-platoon-logo-black.png', 14, 'Code Platoon helps Veterans, active duty Servicemembers, and military spouses transition into the civilian workforce by providing technical training and career placement. Our training programs are designed specifically for members of the military community. They feature immersive curriculums combined with career preparation and coaching, professional networking, and opportunities for paid apprenticeships after graduation. \n\nSince Code Platoon launched in 2016, we have graduated more than 200 students into new careers in software engineering. More than 80% of our graduates are employed within six months of completion. Our graduates are more than simply employed; they have a median salary of $72,000 within six months of graduation. After 24 months, their median salary is $96,000.', NULL);
...
INSERT INTO `bootcamp_review` (`id`, `user_id`, `bootcamp_id`, `overall_rating`, `instructors_rating`, `curriculum_rating`, `job_assistance_rating`, `graduation_date`, `bootcamp_review`, `review_date`) VALUES (DEFAULT, DEFAULT, DEFAULT, NULL, NULL, NULL, NULL, NULL, NULL, '1000-01-01 00:00:00');
...
INSERT INTO `reply` (`id`, `post_id`, `user_id`, `reply_id`, `reply`, `created`, `updated`, `enabled`) VALUES (DEFAULT, DEFAULT, DEFAULT, NULL, NULL, NULL, NULL, NULL);
...
INSERT INTO `bootcamp_tech_joiner` (`bootcamp_id`, `bootcamp_tech_id`) VALUES (DEFAULT, DEFAULT);
INSERT INTO `bootcamp_tech_joiner` (`bootcamp_id`, `bootcamp_tech_id`) VALUES (DEFAULT, DEFAULT);
